...
def generate_products_via_gemini(
    ...
    prompt = f"""あなたは「一人暮らし研究室」というWEB雑誌の編集長です。
20〜30代のワンルーム・一人暮らし読者に向けて、「QOL（生活の質）向上」「狭い部屋の収納」「時短・タイパ」を軸にした商品を選んで紹介しています。
今日は{month}月（{season}）です。
今月のテーマ: {theme}
今月のイベント: {', '.join(events)}{exclusion_note}{weekly_hint}

上記の文脈で「ワンルーム一人暮らしの生活ストレスを解決する」商品を{count}件推薦してください。
・ターゲット: 20〜30代の一人暮らし男女（ワンルーム・1K在住）
・重視するテーマ: 狭い部屋の収納、生活動線の改善、家事時短、騒音対策、食事まわり、睡眠の質
・スペック（何グラム、何ワット等）より「生活がどう変わるか」で選ぶ
...
[
  {{
    ...
    "category": "カテゴリ名（収納・整理/キッチン/インテリア/ガジェット/生活家電/睡眠・リラックス のいずれか）",
    "features": ["特徴1（生活がどう変わるか）", "特徴2", "特徴3"],
    "why_viral": "一人暮らしの人がこれに反応する理由（50文字以内、ワンルーム・QOL文脈で）",
    "story_hook": "ワンルーム住民が「わかる…」と思う共感フックの一文（30文字以内）",
    "user_problem": "この商品が解決するストレス・悩み（20文字以内）"
  }}
]
